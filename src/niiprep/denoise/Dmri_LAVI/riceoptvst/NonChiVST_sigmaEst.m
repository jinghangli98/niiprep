function sigma_hat= NonChiVST_sigmaEst(z,printOut,est_type,VST_ABC,L,maxIter,r_tolerance)
...
while (jjjjj<maxIter)&&(converged==0)
    ...
    if exist('sigma_hat','var')  %% the variable exists after the first iteration
        ...
        sigma_hat=sigma_hat*function_stdEst((sqrt(max((z.^2)./(sigma_hat.^2)-L,0))),kernel_type,1,est_type);
        
    else  %% used at iteration 1
          sigma_hat=function_stdEst(z,kernel_type,1,est_type);
    end
