...
%dim_to_replicate = [(floor((x1 -x2)/2)+rem((x1 -x2),2)) floor((y1 -y2)/2) floor((z1 -z2)/2)];

%z = padarray(z,dim_to_replicate,'replicate','both');


%sigma_hat_map = ones(size(z));
...
elseif est_type==2    %%% mean of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat = (abs(z)); %  assumes, for simplicity, that mean(z(:))=0.
    end
