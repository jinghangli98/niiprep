function [y_est, sigma_est] = bm4d(z, distribution, sigma, profile, do_wiener, do_previous, I_ref, verbose)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if ~exist('do_previous','var') || isempty(verbose)
    do_previous = 1;
end
if ~exist('I_ref','var') || isempty(verbose)
    I_ref = 0;
    do_previous = 0;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sharpening           = 0.95;    % sharpening alpha parameter (1 -> no sharpening)
...
% Modified profile VINICIUS
if strcmpi(profile,'vini4d')
    N1               = 4;
    N3               = min(N1,size(z,3)); 
    ...
    %thresholding         = 'soft'; % tresholding type
...
if strcmpi(profile,'mp')
    ...
    lambda_thr4D     = 2.8;%2.8
    tau_match        = 0.2;%0.2
    tau_match_wiener = 0.1;%0.1
...
%figure, imshow([sigma_hat(:,:,1) sigma_est(:,:,1)],[]);colormap(gca,jet);colorbar
if verbose
