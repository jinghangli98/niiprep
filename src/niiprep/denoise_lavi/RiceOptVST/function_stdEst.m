function sigma_hat=function_stdEst(z,kernel_type,scalar,est_type,which_dims)
...
%                0 => outputs a map/matrix (same size of the input)
%                this is to be used together with the homomorphic approach
%
% 
...
%altered by Vinicius P. Campos
%When scalar=0 , the convolution is of shape = 'same'
for jj=which_dims
   if scalar==1
       z=convn(z,permute(kernel,circshift((1:max(2,jj)),[0 jj-1])),'valid');
   else
       z=convn(z,permute(kernel,circshift((1:max(2,jj)),[0 jj-1])),'same');
   end
end
  
%altered by Vinicius P. Campos
%When scalar=0 , returns the map of sigma_hat
if est_type==1        %%% median of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat = ((z)); %  assumes, for simplicity, that median(z(:))=0.  
    end
...
elseif est_type==2    %%% mean of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat = ((z)); %  assumes, for simplicity, that mean(z(:))=0.
    end
...
elseif est_type==3     %%% sample standard deviation
    ...
    if scalar==1
        ...
        sigma_hat = sqrt(abs(z).^2); %  assumes, for simplicity, that mean(z(:))=0.
    end
